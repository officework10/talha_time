<div>
    ...
    <!-- Toast Notification - FINAL WORKING VERSION -->
    <div x-data="{
        show: false,
        message: '',
        init() {
        ...
        class="fixed top-6 right-6 bg-[#55be30] text-white px-4 py-2 rounded-lg shadow-lg z-50">
        <span x-text="message"></span>
    </div>

    @if ($detail)
        <div class="lg:flex md:flex flex px-5 mb-3 bg-[#55be30] text-white rounded-lg items-center mt-2 justify-between">
            <div id="actual_feedback" x-data="{
                submitted: false,
                loading: false,
                success: false,
                init() {
                    Livewire.on('feedbackSaved', () => {
                        this.loading = false;
                        this.success = true;
                    });
                }
            }">
                <div class="md:flex items-center text-center flex-wrap">
                    <div>
                        <template x-if="!submitted">
                            <div class="md:flex items-center text-center flex-wrap">
                                <strong class="me-md-2 text-[16px]">Was this helpful?</strong>
                                <div class="bg-gray rounded flex items-center text-[13px]">
                                    <button type="button" wire:click="submitFeedback('like')"
                                        @click="submitted = true; loading = true"
                                        class="flex-grow-1 cursor-pointer text-end px-2 py-1" id="like">
                                        <span x-show="!loading">👍 Like</span>
                                        <span x-show="loading"
                                            class="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full ml-1"></span>
                                    </button>

                                    <div class="mx-1">|</div>

                                    <button type="button" wire:click="submitFeedback('dislike')"
                                        @click="submitted = true; loading = true"
                                        class="flex-grow-1 cursor-pointer text-start pl-2 px-2 py-1" id="dislike">
                                        <span x-show="!loading">👎 Dislike</span>
                                        <span x-show="loading"
                                            class="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full ml-1"></span>
                                    </button>
                                </div>
                        ...
                        <template x-if="submitted && loading">
                            <p class="text-white text-sm font-semibold my-2">Loading...</p>
                        </template>

                        <template x-if="success">
                            <p class="text-white text-sm font-semibold my-2">✅ Thanks for your feedback!</p>
                        </template>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-center p-2">
                <button wire:click="openShareModal"
                    class="me-2 p-2 text-[15px] rounded-lg cursor-pointer hover:bg-[#e3e3e3] bg-white flex items-center justify-center">
                    <img width="18" alt="Share" src="{{ asset('assets/new-share-CJ_G3xQj.png') }}">
                </button>
                <button
                    class="open-feedback-btn mx-md-1 p-2 text-[15px] rounded-lg cursor-pointer hover:bg-[#e3e3e3] bg-white flex items-center justify-center">
                    <img width="20" alt="Report" src="{{ asset('assets/report-DErXaa7-.png') }}">
                </button>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div x-data @click.outside="$wire.closeShareModal()"
                ...
                <button wire:click="closeShareModal"
                    class="absolute top-3 right-3 text-gray-500 hover:text-black focus:outline-none">
                ...
                <p class="text-xl font-bold text-center mb-4">Share the {{ $calculatorName  }}</p>
                <!-- Social Media Icons -->
                <div class="flex justify-center space-x-4 mb-6">
                    <button wire:click="share('facebook')"
                        class="bg-[#55be30] rounded-full p-2 text-white hover:[#457534]">
                ...
                <!-- Copyable Link -->
                <div x-data="{ showToast: false, shareUrl: window.location.href }"
                    x-on:link-copied.window=" navigator.clipboard.writeText(shareUrl); showToast = true; setTimeout(() => showToast = false, 2500);"
                    class="flex items-center bg-gray-100 p-3 rounded-lg relative">
                    <!-- Show Actual URL from Browser -->
                    <input x-model="shareUrl"
                        class="flex-grow bg-transparent border-none input_border text-gray-600 px-4" readonly>
                    <button wire:click="copyLink" class="ml-2 text-white rounded-lg px-2 py-1 focus:outline-none">
                        <img src="{{ asset('assets/images/copy.svg') }}" alt="Copy">
                    </button>
                    <!-- Toast -->
                    <div x-show="showToast" x-transition
                        class="absolute -top-12 right-0 bg-black text-white px-4 py-2 rounded-md text-sm shadow-lg z-50">
                        ✅ Link copied!
                    </div>
                </div>
