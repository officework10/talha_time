...
function JSgCalc (element){
	...
	//Draws thge vertex of an equation (i.e. when it changes direction)
		...
		var f = Calc.makeFunction(equation);
		var scale = this.getScale();
		var xpos = x / scale.x + this.currCoord.x1 - (this.startCoord.y2 - this.startCoord.y1);
	...
	this.drawDerivative = function(equation, color, x) {
		...
		var scale = this.getScale();
		var xpos = Calc.roundFloat(this.arbRound(x / scale.x + this.currCoord.x1 - (this.startCoord.y2 - this.startCoord.y1), this.xgridscale/100));
	...
	this.checkMove = function(x, y) {
		...
		if(this.mousebutton === 1) {
			...
			this.draw();
			var gety=this.startCoord.y2 - this.startCoord.y1;
			// this.drawTrace(this.getEquation(jsgui.currEq), this.getColor(jsgui.currEq), (x / scale.x + this.currCoord.x1)-19.600746268656717);
			this.drawTrace(this.getEquation(jsgui.currEq), this.getColor(jsgui.currEq), (x / scale.x + this.currCoord.x1)-gety);
		}
	...
	this.resizeGraph = function(width, height) {
		...
		this.maxgridlines.x = 0.055 * width;
		this.maxgridlines.y = 0.055 * height;
		this.draw();
	};
	...
	this.initCanvas = function() {
		if (this.graph.getContext){
			...
			this.resizeGraph($("#graph_wrapper").innerWidth(), $("#sidewrapper").height()+100);
			this.currCoord = {x1 : -10 * (this.width / this.height), y1 : -10, x2 : 10 * (this.width / this.height), y2 : 10};
			this.startCoord = this.copyCoord(this.currCoord);
			jsgui.evaluate();
