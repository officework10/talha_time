...
; (function ($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        var t = new Date();
        // console.log('date=> '+t);
        t = new Date(c_date[0] + ' ' + c_date[1] + ' ' + c_date[2] + ' ' + c_date[3]);
        // console.log('date2=> '+t);
        this.currentDate = new Date(c_date[0] + ' ' + c_date[1] + ' ' + c_date[2] + ' ' + c_date[3]);
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            var header = $('<header>' +
                '<b><p class="month"></p></b>' +
                '<a class="btn btn-prev mt-1" href="#"><img src="' + arrow_left + '" alt="arrow left" width="18px" height="18px"></a>' +
                '<a class="btn btn-next mt-1" href="#"><img src="' + arrow_right + '" alt="arrow right" width="18px" height="18px"></a>' +
                '</header>');
        ...
        buildCalendar: function (fromDate, calendar) {
            ...
            var body = $('<table class="col-12 radius-10" cellscaping="0"></table>');
            var thead = $('<tr class="bg-light-blue label radius-10 headings"></tr>');
            var tbody = $('<tbody></tbody>');

            //Header day in a week ( (1 to 8) % 7 to start the week by monday)
            for (var i = 1; i <= this.settings.days.length; i++) {
                thead.append($('<td><b>' + this.settings.days[i % 7].substring(0, 3) + '</b></td>'));
            }
